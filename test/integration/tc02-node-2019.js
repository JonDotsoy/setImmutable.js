...
/**
 * TC02 -- the same checks as TC01 (test/integration/tc01-node-2017.js),
 * run against Node 10 instead: the LTS release ("Dubnium") active for
 * most of 2019 -- Active LTS through April 2019, then Maintenance LTS
 * for the rest of the year. Unlike Node 6/2017, there's no repo-history
 * anchor for this version (the project's own commits stop in 2017);
 * Node 10 was picked as representative of what a typical production
 * project had pinned during 2019. Run with plain `node` (no
 * mocha/babel/TS -- this repo's own tooling targets much newer Node),
 * against the package as installed from the npm registry, not the
 * repo's src/.
 *
 * See .github/workflows/test.yml (job "smoke-node-2019") for how this
 * is invoked in CI.
 */
...
console.log('TC02: setimmutable is importable on Node ' + process.version)
...
console.log(
  'TC02: all cases passed on Node ' + process.version +
