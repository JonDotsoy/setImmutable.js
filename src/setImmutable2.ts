import setImmutable = require('./setImmutable')

/**
 * MVP of a real, path-aware return type for setImmutable -- covers the two
 * path forms whose keys resolve statically (dot-separated strings and
 * arrays of keys). Bracket/numeric-index paths in string form (e.g.
 * "list[1]", "a[2].b") aren't parsed here -- they'd need real bracket
 * parsing in Split below, which is a separate, harder piece of work -- and
 * a customClone that changes the constructor along the way can't be typed
 * at all, so both fall back to `any` via the Path type param's constraint.
 */
...
function setImmutable2<T, P extends string | readonly (string | number)[], V> (
  obj: T,
  path: P,
  val: V,
